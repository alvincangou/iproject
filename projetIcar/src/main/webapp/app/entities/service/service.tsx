  ...
  return (
    <div>
      <h2 id="service-heading">
        <Translate contentKey="icarApp.service.home.title">Services</Translate>
        <Link to={`${match.url}/new`} className="btn btn-primary float-right jh-create-entity" id="jh-create-entity">
          <FontAwesomeIcon icon="plus" />
          &nbsp;
          <Translate contentKey="icarApp.service.home.createLabel">Create new Service</Translate>
        </Link>
      </h2>
        ...
        {serviceList && serviceList.length > 0 ? (
            ...
            <tbody>
              {serviceList.map((service, i) => (
                <tr key={`entity-${i}`}>
                  ...
                  <td>{service.name}</td>
                  <td>{service.necessaryTime}</td>
                  <td>{service.price}</td>
                  <td>{service.ability ? <Link to={`freelance/${service.ability.id}`}>{service.ability.id}</Link> : ''}</td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="icarApp.service.home.notFound">No Services found</Translate>
            </div>
          )
        )}
      </div>
